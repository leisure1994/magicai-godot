...
func _handle_command(cmd: Dictionary) -> Dictionary:
	...
	print("[MagicAI] 收到指令: ", action, " | 参数: ", cmd)
	...
	match action:
		...
		"delete_node":
			...
			if target:
				target.queue_free()
				return {"ok": true, "message": "节点已删除: " + cmd.get("name", "")}
		...
		"play":
			...
			return {"ok": true, "message": "场景已启动运行"}
		...
		"open_scene":
			...
			return {"ok": true, "message": "已打开: " + path}
		...
		"write_script":
			...
			if path == "" or code == "":
				return {"ok": false, "error": "path 和 code 不能为空"}
			var file = FileAccess.open(path, FileAccess.WRITE)
			if not file:
				return {"ok": false, "error": "无法写入文件: " + path}
			file.store_string(code)
			file.close()
			return {"ok": true, "message": "脚本已写入: " + path}
